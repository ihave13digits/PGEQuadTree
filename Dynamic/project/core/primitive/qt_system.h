...
namespace QT { int point_count=0, born=0, dead=0; float curve=32.0; }
#include "point.h"
namespace QT { Point*    closest = new Point(0, 0); }
#include "quad.h"
...
namespace QT
{
    ...
    void Update(float dt)
    {
        ...
        if (var::update_tree)
        {
            ...
            float a = std::max(point_count, 16) * float(point_count)/(float(point_count)*curve);
